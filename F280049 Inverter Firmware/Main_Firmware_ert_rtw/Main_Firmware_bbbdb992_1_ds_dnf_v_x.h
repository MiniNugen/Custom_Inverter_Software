/* Simscape target specific file.
 * This file is generated for the Simscape network associated with the solver block 'Main_Firmware/FOC test/Solver Configuration'.
 */
/* Main_Firmware_bbbdb992_1_ds_dnf_v_x.h - header for method Main_Firmware_bbbdb992_1_ds_dnf_v_x */
#ifdef __cplusplus

extern "C"
{

#endif

#ifndef MAIN_FIRMWARE_BBBDB992_1_DS_DNF_V_X_H
#define MAIN_FIRMWARE_BBBDB992_1_DS_DNF_V_X_H 1

  int32_T Main_Firmware_bbbdb992_1_ds_dnf_v_x(const NeDynamicSystem *sys, const
    NeDynamicSystemInput *Q, NeDsMethodOutput *M);

#endif                       /* #ifndef MAIN_FIRMWARE_BBBDB992_1_DS_DNF_V_X_H */

#ifdef __cplusplus

}

#endif
